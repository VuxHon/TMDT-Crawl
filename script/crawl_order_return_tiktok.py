...
def process_order_return(offset):
    ...
    for order in order_return:
        ...
        for product in products:
            ...
            if not existing_sku:
                # SKU doesn't exist, need to insert it
                logging.info(f"SKU {sku_id} not found in ecom_sku_products, inserting...")
                
                # Get product_name, sku_name, and product_image from product object
                product_name = product.get('product_name')
                sku_name = product.get('sku_name')
                product_image_url = None
                if product.get('product_image') and 'url_list' in product['product_image']:
                    product_image_url = product['product_image']['url_list'][0] if product['product_image']['url_list'] else None
                
                if not product_name:
                    logging.warning(f"product_name not found for sku_id {sku_id}, skipping SKU insert")
                else:
                    # Find product_id from ecom_products by name
                    find_product_query = """
                        SELECT id FROM ecom_products WHERE name = %s LIMIT 1
                    """
                    product_result = pg.fetch_one(find_product_query, (product_name,))
                    
                    if product_result:
                        product_id = product_result['id']
                        
                        # Insert new SKU into ecom_sku_products
                        insert_sku_query = """
                        ...
                        """
                        pg.execute(insert_sku_query, (product_id, sku_id, sku_name, product_image_url))
                        logging.info(f"Inserted SKU {sku_id} into ecom_sku_products with product_id {product_id}")
                    else:
                        logging.warning(f"Product with name '{product_name}' not found in ecom_products, cannot insert SKU {sku_id}")
...
while page > 0:
    try:
        ...
        logging.info("retry with offset: {offset}")
        pass
